...
let manuLi =document.querySelectorAll('header ul li a');
let section = document.querySelectorAll('section');

function activeManu(){
  let len = section.length;
    while (--len && window.scrollY + 97 < section[len].offsetTop){}
    manuLi.forEach(sec => sec.classList.remove("active"));
    manuLi[len].classList.add("active");
  }

activeManu();
window.addEventListener("scroll", activeManu);
